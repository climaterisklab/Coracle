...
all_bleach_info <- all_bleach_info %>% filter(Bleaching_Level == "Population")
...
all_bleach_info <- all_bleach_info %>%
  group_by(Site_ID, Date_Year, Date_Month, Percent_Bleached) %>% 
  summarise(
...
bleach_counts <- Site_Info_tbl_final %>%
  ...
  summarise(
    bleach_events = sum(!is.na(Percent_Bleached), na.rm = TRUE),
    .groups = "drop"
  )
all_bleach_info <- all_bleach_info %>%
...
write_csv(Site_Info_tbl_final, "GBCD_full_data_cleaned.csv")               ## including population, colony, and na
write_csv(all_bleach_info, "All_Bleaching_Events_Data.csv")                  ## only population level bleaching events
