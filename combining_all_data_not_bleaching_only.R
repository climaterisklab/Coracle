...
# allMermaidSampEventsTBL <- allMermaidSampEventsTBL %>%
#   filter(if_any(starts_with("colonies_bleached"), ~ !is.na(.x)))
...
Combined_Bleaching_Data <- bind_rows(
  ...
  Site_Info_tbl_final %>%
    select(Site_ID, Latitude_Degrees, Longitude_Degrees, Date_Year, Date_Month, Date_Day,
           Percent_Bleached, Ecoregion_Name, Country_Name, Source)
)

# Check the combined data
cat("Total observations:", nrow(Combined_Bleaching_Data), "\n")
